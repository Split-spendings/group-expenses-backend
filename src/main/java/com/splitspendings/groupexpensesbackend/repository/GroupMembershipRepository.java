...
public interface GroupMembershipRepository extends JpaRepository<GroupMembership, Long> {
    ...
    @Query("select gm from GroupMembership gm where gm.appUser.id = :app_user_id and gm.active=:isActive")
    List<GroupMembership> findAllByAppUserIdAndIsActive(@Param("app_user_id") UUID appUserId, @Param("isActive") boolean isActive);

    @Query("select gm from GroupMembership gm where gm.appUser.id = :app_user_id")
